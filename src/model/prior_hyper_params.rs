...
use crate::{
	model::{ClusterSizePrior, NumClustersPrior},
	utils::Array1Wrapper,
	utils::get_rng,
};
...
pub struct PriorHyperParams {
	/// Shape hyperparameter for the $\mathrm{Gamma}$ likelihood on intra-cluster
	/// dissimilarities. Smaller values lead to greater within-cluster
	/// cohesion.
	delta1: f64,

	/// Shape hyperparameter for the $\mathrm{Gamma}$ likelihood on inter-cluster
	/// dissimilarities. Larger values lead to greater inter-cluster repulsion.
	delta2: f64,

	/// Shape hyperparameter for the $\mathrm{Gamma}$ prior on
	/// the rate parameters of the $\mathrm{Gamma}$ likelihood
	/// for intra-cluster dissimilarities.
	/// Smaller values for $\alpha$ lead to more cohesion and less variability
	/// within clusters.
	alpha: f64,

	/// Rate hyperparameter for the $\mathrm{Gamma}$ prior on the
	/// rate parameters of the $\mathrm{Gamma}$ likelihood on intra-cluster distances.
	/// Larger values for $\beta$ lead to less cohesion and greater variability
	/// within clusters.
	beta: f64,

	/// Rate hyperparameter for the $\mathrm{Gamma}$ prior on the
	/// rate parameters of the $\mathrm{Gamma}$ likelihood on inter-cluster distances.
	...
	/// Shape hyperparameter for the $\mathrm{Gamma}$ prior on the stopping count
	...
	/// Rate hyperparameter for the $\mathrm{Gamma}$ prior on the stopping count r
	/// in the negative binomial likelihood for each of the cluster sizes.
	...
	/// Hyperparameter in the $\mathrm{Beta}$ prior for the prior on the success
...
impl PriorHyperParams {
	/// Set the hyperparameter $\delta_1$.
	...
	/// Set the hyperparameter $\delta_2$.
...
impl PriorHyperParams {
	...
	/// Convert the `PriorHyperParams` object to a Python dictionary.

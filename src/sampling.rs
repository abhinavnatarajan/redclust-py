...
impl MCMCState {
	...
	pub(crate) fn sample_r_conditional<R: Rng>(
		...
		let proposal_distr = unsafe { LogNormal::new(r, proposalsd_r).unwrap_unchecked() };
		let r_proposed = proposal_distr.sample(rng);
		let reverse_distr = unsafe { LogNormal::new(r_proposed, proposalsd_r).unwrap_unchecked() };
		...
		clust_list.iter().for_each(|&j| {
			unsafe {
				ln_prob_proposal +=
					ln_gamma(*clust_sizes.get_unchecked(j as usize) as f64 + r_proposed - 1.0);
				ln_prob_current +=
					ln_gamma(*clust_sizes.get_unchecked(j as usize) as f64 + r - 1.0);
			}
		});
		...
		let toss = unsafe { Uniform::new(0.0, 1.0).unwrap_unchecked().sample(rng).ln() };
